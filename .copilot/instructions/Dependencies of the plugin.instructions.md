...
| 2026-08-13 | 1.133 | ✅ 全部兼容（源码 + 本机实测） | ① release notes 审查无 Tasks/Profile/扩展存储/Settings Sync 变更；② 本机实测 1.133.0：`storage.json` 的 `userDataProfiles`(4)/`profileAssociations.workspaces`(10)/`lastActiveWindow` 均正常，`submenuitem.Profiles` 仍无（死代码）；③ 源码确认 `extensionsResource`/`mcpResource`（`userDataProfile.ts`）与 `DISABLED_EXTENSIONS_STORAGE_PATH='extensionsIdentifiers/disabled'`（`extensionManagement.ts`）未变；④ 实测 `state.vscdb` 的 `ItemTable (key TEXT UNIQUE ON CONFLICT REPLACE, value BLOB)` 与 1.132.1 一致；⑤ 本机 1.8.6 激活正常（onStartupFinished 无 error 日志）；⑥ `engines.vscode ^1.131.0` 兼容 1.133 无需升级 |
| 2026-08-21 | 1.134 | ✅ 全部兼容（源码 + 本机实测，插件 1.9.0） | ① release notes 聚焦 Agents/Chat/Editor，无 Profile/扩展存储/Settings Sync 变更，无 Deprecated 项；② 本机实测：`storage.json` 的 `userDataProfiles`(4)/`workspaces`(13)/`emptyWindows`(1)/`lastActiveWindow.backupPath` 均正常，`submenuitem.Profiles` 仍无；③ 源码确认 `extensionsResource`/`mcpResource`（`userDataProfile.ts`）、`DISABLED_EXTENSIONS_STORAGE_PATH='extensionsIdentifiers/disabled'`（`extensionManagement.ts`）、`saveStoredProfileAssociations`、`windowsStateStorageKey` 均未变；④ `state.vscdb` 实测 `ItemTable (key TEXT UNIQUE ON CONFLICT REPLACE, value BLOB)` 未变，禁用键存在；⑤ 本机 1.9.0 激活正常（exthost.log onStartupFinished 无 error），parents 扁平键/ignoredSettings 4 key/Sync 快照均正常；⑥ `engines.vscode ^1.131.0` 兼容 1.134 无需升级 |
